...
export default async function PerPersonRollupPage({
  ...
  if (!targetUser) {
    return (
      <div className="p-8 text-center bg-white border border-gray-100 rounded-2xl max-w-lg mx-auto mt-12 shadow-sm font-ibmsans text-ink space-y-3">
        <User className="w-10 h-10 text-gray-400 mx-auto" />
        <h2 className="text-lg font-bold font-ibmserif text-ink">Employee Not Found</h2>
  ...
  return (
    <div className="space-y-8 font-ibmsans max-w-6xl mx-auto py-4">
      {/* Privacy Notice Banner */}
      <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl px-5 py-3.5 flex items-center justify-between shadow-xs">
        ...
        <span className="text-2xs font-bold uppercase tracking-wider font-ibmmono text-amber-700 bg-amber-100 px-2 py-0.5 rounded">
          HR Confidential
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-accent/10 border border-accent/20 flex items-center justify-center text-accent text-xl font-bold font-ibmserif shrink-0">
            {targetUser.name ? targetUser.name.charAt(0).toUpperCase() : 'E'}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-display font-extrabold text-ink">{targetUser.name}</h1>
        ...
        <div className="flex items-center gap-3 text-xs font-bold font-ibmmono">
          ...
          <div className="px-3.5 py-2 rounded-xl bg-accent/10 border border-accent/20 text-center">
            <div className="text-2xs text-accent uppercase">Beneficiary (For)</div>
            <div className="text-base text-accent font-extrabold mt-0.5">{asBeneficiaryCount}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
          <div className="text-2xs font-extrabold text-muted uppercase tracking-wider font-ibmmono">Total Approvals</div>
          <div className="text-2xl font-black text-ink font-ibmserif mt-1">{totalCount}</div>
        </div>
        <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4 shadow-sm">
          <div className="text-2xs font-extrabold text-emerald-700 uppercase tracking-wider font-ibmmono flex items-center gap-1">
            <CheckCircle2 className="w-3.5 h-3.5" /> Approved
          </div>
          <div className="text-2xl font-black text-emerald-800 font-ibmserif mt-1">{approvedCount}</div>
        </div>
        <div className="bg-amber-50/50 border border-amber-100 rounded-2xl p-4 shadow-sm">
          <div className="text-2xs font-extrabold text-amber-700 uppercase tracking-wider font-ibmmono flex items-center gap-1">
            <Clock className="w-3.5 h-3.5" /> Pending
          </div>
          <div className="text-2xl font-black text-amber-800 font-ibmserif mt-1">{pendingCount}</div>
        </div>
        <div className="bg-red-50/50 border border-red-100 rounded-2xl p-4 shadow-sm">
          <div className="text-2xs font-extrabold text-red-700 uppercase tracking-wider font-ibmmono flex items-center gap-1">
            <XCircle className="w-3.5 h-3.5" /> Rejected
          </div>
          <div className="text-2xl font-black text-red-800 font-ibmserif mt-1">{rejectedCount}</div>
        </div>
      </div>
      ...
      {Object.keys(categoryCounts).length > 0 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted font-ibmmono">Category Breakdown</h3>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden space-y-0">
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-6 py-4">
                        {isBeneficiary ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-2xs font-bold bg-accent/10 text-accent font-ibmmono">
                            Beneficiary (For)
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-2xs font-bold bg-gray-100 text-gray-600 font-ibmmono">
                            Requester
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-2xs font-bold uppercase tracking-wider font-ibmmono ${
                      ...
                      <td className="px-6 py-4">
                        {val.state !== 'NOT_APPLICABLE' && (
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-2xs font-bold border font-ibmmono ${val.badgeClass}`}>
                            {val.label}
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <Link href={`/${tenant}/requests/${r.id}`} className="text-accent font-bold hover:underline">
                          View →
                        </Link>
                      </td>
                    </tr>
